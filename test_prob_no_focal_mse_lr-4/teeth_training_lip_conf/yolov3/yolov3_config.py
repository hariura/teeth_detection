...
def compute_loss(pred, conv, label, bboxes, i=0, CLASSES=YOLO_COCO_CLASSES):
    ...
    # config
    giou_loss = 1 * respond_bbox * bbox_loss_scale * (1 - giou)
    ...
    conf_loss =   10*(
            ...
            +
            respond_bgd * tf.reshape(tf.keras.losses.MSE(tf.sigmoid(conv_raw_conf), respond_bbox), list(respond_bbox.shape))
    )
    # config
