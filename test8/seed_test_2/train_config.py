...
#     tf.keras.utils.set_random_seed(SEED)
    ...
    ##
    global_lr = tf.Variable(1e-7, trainable=False, dtype=tf.float32)
    ##
    ...
    ### add_start ###
    print("frist","\n")
    print("the number of layers",len(yolo.get_weights()))
    l = 0
    for i in yolo.get_weights():
        l +=sum(i.reshape([-1]))
    print("all weight sum is : ",l)
...
#         random.seed(SEED)
        ...
        with tf.GradientTape() as tape:
            ...
            print("gradient : ",np.float32(sum(gradients[1])))
...
#             optimizer.lr.assign(lr.numpy())
    ...
    for epoch in range(TRAIN_EPOCHS):
        for image_data, target in trainset:
            ...
            print("epoch:{:2.0f} step:{:5.0f}/{}, lr:{:.20f}, giou_loss:{:7.10f}, conf_loss:{:7.10f}, prob_loss:{:7.10f}, total_loss:{:7.10f}"
