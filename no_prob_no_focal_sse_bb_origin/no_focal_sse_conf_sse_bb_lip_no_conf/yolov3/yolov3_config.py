...
from yolov3.utils_config import read_class_names
...
def compute_loss(pred, conv, label, bboxes, i=0, CLASSES=YOLO_COCO_CLASSES):
    ...
    # config
    conf_loss =   (
            respond_bbox * tf.reshape(tf.keras.losses.MSE(tf.sigmoid(conv_raw_conf), respond_bbox), list(respond_bbox.shape))
            +
            0.5*respond_bgd * tf.reshape(tf.keras.losses.MSE(tf.sigmoid(conv_raw_conf), respond_bbox), list(respond_bbox.shape))
    )
    # config
#    prob_loss =  respond_bbox * tf.reshape(tf.keras.losses.MSE(tf.sigmoid(conv_raw_prob), label_prob), list(respond_bbox.shape))
