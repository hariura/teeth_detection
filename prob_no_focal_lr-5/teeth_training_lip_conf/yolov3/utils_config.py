...
from yolov3.yolov4_config import *
...
def postprocess_boxes_excel(pred_bbox, original_image, input_size, score_threshold):
    valid_scale=[0, np.inf]
    pred_bbox = np.array(pred_bbox)
    ...
    return np.concatenate([coors, scores[:, np.newaxis], classes[:, np.newaxis]], axis=-1), pred_coor, pred_conf, pred_prob
